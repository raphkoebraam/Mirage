...
- **The device is the only positional argument.** Every other value is a named option, so invocations read unambiguously and the device can always be omitted to target the booted simulator: `create --name` (optional; defaults to the device type's name), `clone --name`, `rename --name`, `upgrade --runtime`, `app install --path`, `app uninstall/launch/terminate/info --bundle-id`, `app container --bundle-id [--container]`, `app install-data --path`, `media add --path`, `open --url`, `push --bundle-id --payload`, `getenv --variable`, `logverbose --on/--off`, `keychain add-cert/add-root-cert --path`, `location set --latitude --longitude`, `location run --scenario`, `spawn --executable`, `runtime install --platform --version`. `ui appearance/content-size/increase-contrast` read by default and write with `--set`. `privacy` is now `privacy grant|revoke|reset --service [--bundle-id]`. Pass-through arguments (`app launch`, `spawn`) still follow `--`.
